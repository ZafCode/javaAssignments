...
public class Shelf {
    ...
    public static boolean addBook(int id, String author, String title, int pages ){
    	...
    	boolean didBookAdded = true;
    	
    	if (!isIDUnique(id) || title.equals("")) {
			didBookAdded = false ;
		}else if(pages<=0) {
			didBookAdded = false ;
		}else {
     ...
     * Puslu Kitalar Atlasi   atlasi   findBookByPartialTitle("atLASi") atlasi
     */
    ...
    public static List<Book> getAllBooksForAuthor(String author) {
       ...
       for (int i = 0; i < getTheBooks().size(); i++) {
   		if (getTheBooks().get(i).getAuthor().toLowerCase().equals(author.toLowerCase())) {
				books.add(getTheBooks().get(i));
			}
		}
    ...
    public static void removeBook(int id) {
    	...
    	for (int i = 0; i < getTheBooks().size(); i++) {
    		if (getTheBooks().get(i).getId()==id) {
				theBooks.remove(getTheBooks().get(i));
			}
		}
