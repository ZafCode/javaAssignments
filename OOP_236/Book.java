...
public class Book {
    ...
    public void setAuthor(String author) {
        ...
        if (author.equals("")) {
			...
			if(authorWords.length == 1) {
				...
				for (String string : authorWords) {
					resultAuthor += capitalize(string)+" ";
				}
				this.author = resultAuthor.trim();
			}
		}
    ...
    public void setTitle(String title) {
    	...
    	if (title.equals("")) {
			...
			for (int i = 0; i < titleWords.length; i++) {
				if (!isTitleWord(titleWords[i]) && i!=0) {
					resultTitle += titleWords[i].toLowerCase()+" ";
				}else {
					resultTitle += capitalize(titleWords[i])+" ";
				}
			}
			
			this.title = resultTitle.trim();
		}
    ...
    private int pages;*/
       ...
       return "Book[author="+this.author+", title="+title+", id="+id+", pages="+pages+"]";
